

# Hi, I'm Krzysztof 👋

Automation and Robotics student at AGH University of Science and Technology, focused on DevOps and Data Engineering.  
Interested in Python, SQL, Linux, Docker, cloud tools, and building practical solutions for automation, data processing, and system integration.


## Tech Stack

<p align="left">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg" width="80" height="80" alt="Python"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/postgresql/postgresql-original.svg" width="80" height="80" alt="PostgreSQL"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/linux/linux-original.svg" width="80" height="80" alt="Linux"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/docker/docker-original.svg" width="80" height="80" alt="Docker"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/azure/azure-original.svg" width="80" height="80" alt="Azure"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/git/git-original.svg" width="80" height="80" alt="Git"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/flask/flask-original.svg" width="80" height="80" alt="Flask"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/html5/html5-original.svg" width="80" height="80" alt="HTML"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/css3/css3-original.svg" width="80" height="80" alt="CSS"/>
  <img src="https://upload.wikimedia.org/wikipedia/commons/c/cf/New_Power_BI_Logo.svg" width="80" height="80" alt="Power BI"/>
</p>


## Focus Areas
- Data pipelines and automation
- Cloud and infrastructure
- Monitoring and logging
- System integration
- DevOps workflows

## Featured Projects

- **Flask Notes API** – simple REST API for note management built with Flask
- **Energy Monitoring System** – data pipeline and analytics project with SQL and Power BI
- **Smart Factory Automation** – industrial IoT simulation in Cisco Packet Tracer

## Contact

<a href="https://www.linkedin.com/in/krzysztof-witek-b4320b315/?locale=pl" target="_blank">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/linkedin/linkedin-original.svg" width="95" alt="LinkedIn" />
</a>

### 🐍 Contribution Snake
![snake svg](https://raw.githubusercontent.com/KrzWit/KrzWit/output/github-snake.svg#gh-light-mode-only)
![snake svg](https://raw.githubusercontent.com/KrzWit/KrzWit/output/github-snake-dark.svg#gh-dark-mode-only)
